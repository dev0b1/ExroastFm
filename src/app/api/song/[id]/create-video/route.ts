...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    // Legacy: previously this route enqueued an ffmpeg/audio job to create a lyric video.
    // We're simplifying the flow: subscribed users receive premium pre-made songs.
    // Keep the old code commented for reference, but return a helpful response.
    ...
    */
    ...
    // New flow: client should call the premium-songs API to retrieve matched pre-made songs.
    return NextResponse.json({ success: true, message: 'legacy_workflow_deprecated' });
  } catch (err) {
